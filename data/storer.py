...
class DataFrameAdapter(ABC):
    @abstractmethod
    def to_sql(self, destination: str = ""):
        pass
...
class CSVStorage(DataStorage[DataFrameAdapter]):
    def save(self, data: DataFrameAdapter, destination: str = "output.csv"):
...
class SQLiteStorage(DataStorage[DataFrameAdapter]):
    ...
    def save(self, data: DataFrameAdapter, destination: str = "prices"):
